...
<%
	...
	ArrayList<SubjectBean> al = new SubjectBeanCl().getAllSub(yuanxi);
...
<form name="form1" method="post">
	...
	<table width="100%" border="1">
		...
		<tr>
			...
			<td colspan="8" scope="col" align="center" height="30px">
				<%
				if(pageCount>8){
				%> <a href=<%="teacher/teacher.jsp?function=sub_all&pageNow=1" %>>[首页]</a>
				<%
				}
				if (pageNow != 1) {
				%> <a
				href=<%="teacher/teacher.jsp?function=sub_all&pageNow=" + (pageNow - 1)%>>[上一页]</a>
				<%
				}
				if (pageCount < 8) {//页面总数少于8，输出所有页码标签
					for (int i = 1; i <= pageCount; i++) {
						...
						%> <label><%=i %></label>&nbsp; <%
						  ...
						  %> <a href=<%="teacher/teacher.jsp?function=sub_all&pageNow=" + i%>>[<%=i%>]</a>&nbsp;
				<%		}
					}
				} else if (pageNow <= 4) {//页面总数大于8，当前页不大于4，输出1~8
					for (int i = 1; i <= 8; i++) {
				 		if(pageNow==i){%> <label><%=i%></label>&nbsp; <%}else{ %> <a
				href=<%="teacher/teacher.jsp?function=sub_all&pageNow=" + i%>>[<%=i%>]</a>&nbsp;
				<%		}
					}
 ...
 %> <a
				href=<%="teacher/teacher.jsp?function=sub_all&pageNow="
									+ i%>>[<%=i%>]</a>&nbsp;
				<%
					}
							}
