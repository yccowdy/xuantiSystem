...
<script type="text/javascript">
	...
	function search() {
		document.form1.action = "TeaManageServlet?flag=screening1";
		document.form1.submit();
	}
</script>
<%
    ...
    ArrayList<SubjectBean> alSub=null;
    alSub=(ArrayList<SubjectBean>) hs.getAttribute("searchResult");
	...
	if(year==null) year="";
	if(alSub==null){
 alSub = new SubjectBeanCl().getSubjectByYear(teacher,selDepartment,key,year);
	}
...
<table width="100%" border="1" >
				...
				<table>
					<tr>
						<td><div>
								...
								&nbsp;&nbsp;题目搜索:<input name="key" type="text" size="10" value="<%=key %>"> &nbsp;
								&nbsp;年份:<input name="year" type="text" size="10" value="<%=year %>"> &nbsp;
							</div></td>
  ...
  SubjectBean sb=alSub.get((pageNow-1)*pageSize+i);
  ...
  <%} %>
	...
	<tr>
			...
			<td colspan="8" scope="col" align="center" height="30px">
				<%
				if(pageCount>8){
				%> <a href=<%="admin/admin.jsp?function=sub_all1&pageNow=1" %>>[首页]</a>
				<%
				}
				if (pageNow != 1) {
				%> <a
				href=<%="admin/admin.jsp?function=sub_all1&pageNow=" + (pageNow - 1)%>>[上一页]</a>
				<%
				}
				if (pageCount < 8) {//页面总数少于8，输出所有页码标签
					for (int i = 1; i <= pageCount; i++) {
						...
						%> <label><%=i %></label>&nbsp; <%
						  ...
						  %> <a href=<%="admin/admin.jsp?function=sub_all1&pageNow=" + i%>>[<%=i%>]</a>&nbsp;
				<%		}
					}
				} else if (pageNow <= 4) {//页面总数大于8，当前页不大于4，输出1~8
					for (int i = 1; i <= 8; i++) {
				 		if(pageNow==i){%> <label><%=i%></label>&nbsp; <%}else{ %> <a
				href=<%="admin/admin.jsp?function=sub_all1&pageNow=" + i%>>[<%=i%>]</a>&nbsp;
				<%		}
					}
 ...
 %> <a
				href=<%="admin/admin.jsp?function=sub_all1&pageNow="
									+ i%>>[<%=i%>]</a>&nbsp;
				<%
					}
							}
	...
	<tr><td colspan="7"><input type="button" value='导出Excel表(请用WPS以上版本打开)' onclick="exportExcel();">计算机科学与技术系共出题:<%=sbbc.get1("计算机科学系") %>&nbsp;网络工程系共出题:<%=sbbc.get1("网络工程系")%>&nbsp;信息安全系共出题:<%=sbbc.get1("信息安全系")%>&nbsp;计算机应用系共出题:<%=sbbc.get1("计算机应用系") %></td></tr>
</table>
</form>